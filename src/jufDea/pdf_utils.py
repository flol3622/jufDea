...
class MyPDF(FPDF):
    def __init__(self):
        ...
        with pkg_resources.as_file(pkg_resources.files(GUI) / "assets" / "SchoolKX_new_SemiBold.ttf") as font_path:
            with open(font_path, 'rb') as src, open(self._font_tempfile.name, 'wb') as dst:
                dst.write(src.read())
...
def add_person_page(pdf, row):
    ...
    with pkg_resources.as_file(pkg_resources.files(GUI).parent / "layout.json") as layout_path:
    ...
    with pkg_resources.as_file(pkg_resources.files(GUI) / "images" / "potloden" / image_rel.name) as image_path:
